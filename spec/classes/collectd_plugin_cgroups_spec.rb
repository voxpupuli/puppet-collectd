  ...
  on_supported_os(baseline_os_hash).each do |os, facts|
    context "on #{os}" do
      ...
      context ':ensure => present, default params' do
        it "creates #{options[:plugin_conf_dir]}/10-cgroups.conf" do
      ...
      context ':ensure => present, specific params, collectd version 5.4.0' do
        ...
        it "creates #{options[:plugin_conf_dir]}/10-cgroups.conf for collectd >= 5.4" do
          is_expected.to contain_file('cgroups.load').with(
            ensure: 'present',
            path: "#{options[:plugin_conf_dir]}/10-cgroups.conf",
      ...
      context ':ensure => absent' do
        ...
        it "does not create #{options[:plugin_conf_dir]}/10-cgroups.conf" do
          is_expected.to contain_file('cgroups.load').with(
            ensure: 'absent',
            path: "#{options[:plugin_conf_dir]}/10-cgroups.conf",
          )
