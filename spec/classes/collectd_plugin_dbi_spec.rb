...
describe 'collectd::plugin::dbi', :type => :class do
  ...
  context ':ensure => present and default parameters' do
    it 'Will create /etc/collectd/conf.d/10-dbi.conf to load the plugin' do
        should contain_file('dbi.load').with({
            :ensure  => 'present',
    ...
    it 'Will create /etc/collectd.d/dbi-config.conf' do
        should contain_concat__fragment('collectd_plugin_dbi_conf_header').with({
          :content => /<Plugin dbi>/,
        })
    end
  end
  ...
  context ':ensure => present and create a db with a query' do
        ...
        :queries => {
          'log_delay'=>{
            ...
            'results'   => [{
                ...
                'instancesfrom'  => ['inet_server_port', 'inet_server_host'],
    ...
    it 'Will create /etc/collectd.d/dbi-config.conf' do
        should contain_concat__fragment('collectd_plugin_dbi_conf_db_mydatabase').with({
        ...
        should contain_concat__fragment('collectd_plugin_dbi_conf_db_mydatabase').with({
            :content => /DriverOption \"host\" \"db\.example\.com\"\n/,
        })
        should contain_concat__fragment('collectd_plugin_dbi_conf_db_mydatabase').with({
            :content => /DriverOption \"username\" \"dbuser\"\n/,
        })
        should contain_concat__fragment('collectd_plugin_dbi_conf_db_mydatabase').with({
            :content => /DriverOption \"password\" \"dbpasswd\"\n/,
        })

        should contain_concat__fragment('collectd_plugin_dbi_conf_query_log_delay').with({
        ...
        should contain_concat__fragment('collectd_plugin_dbi_conf_query_log_delay').with({
            :content => /InstancesFrom \"inet_server_port\" \"inet_server_host\"\n/,
        })
        should contain_concat__fragment('collectd_plugin_dbi_conf_query_log_delay').with({
            :content => /ValuesFrom \"log_delay\"\n/,
        })
    end
  end
