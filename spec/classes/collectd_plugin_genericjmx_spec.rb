...
describe 'collectd::plugin::genericjmx', :type => :class do
  ...
  context 'defaults' do
    ...
    it 'will load the genericjmx plugin' do
      should contain_concat(config_filename).with(:ensure         => 'present',
                                                  :mode           => '0640',
                                                  :owner          => 'root',
                                                  :group          => 'root',
                                                  :ensure_newline => true)
    ...
    it do
      should contain_concat__fragment('collectd_plugin_genericjmx_conf_header').with(:order   => '00',
                                                                                     :target  => config_filename,
                                                                                     :content => /<Plugin "java">.+LoadPlugin "org\.collectd\.java\.GenericJMX".+<Plugin "GenericJMX">/m)
    end

    it do
      should contain_concat__fragment('collectd_plugin_genericjmx_conf_footer').with(:order   => '99',
                                                                                     :target  => config_filename,
