  ...
  on_supported_os(baseline_os_hash).each do |os, facts|
    context "on #{os}" do
      ...
      context ':ensure => present, default params' do
        it "creates #{options[:plugin_conf_dir]}/10-memory.conf" do
      ...
      context ':ensure => present, specific params, collectd version 5.4.2' do
        ...
        it "creates #{options[:plugin_conf_dir]}/10-memory.conf for collectd < 5.5" do
          is_expected.to contain_file('memory.load').with(
            ensure: 'present',
            path: "#{options[:plugin_conf_dir]}/10-memory.conf",
            content: %r{LoadPlugin memory},
        ...
        it "does not include ValuesPercentage in #{options[:plugin_conf_dir]}10-memory.conf" do
      ...
      context ':ensure => present, specific params, collectd version 5.5.0' do
        ...
        it 'creates /etc/collectd.d/10-memory.conf for collectd >= 5.5' do
          is_expected.to contain_file('memory.load').with(
            ensure: 'present',
            path: "#{options[:plugin_conf_dir]}/10-memory.conf",
      ...
      context ':ensure => absent' do
        ...
        it "does not create #{options[:plugin_conf_dir]}/10-memory.conf" do
          is_expected.to contain_file('memory.load').with(
            ensure: 'absent',
            path: "#{options[:plugin_conf_dir]}/10-memory.conf",
          )
