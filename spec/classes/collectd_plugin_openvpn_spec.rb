  ...
  on_supported_os(baseline_os_hash).each do |os, facts|
    context "on #{os}" do
      ...
      context ':ensure => present, default params' do
        it "creates #{options[:plugin_conf_dir]}/10-openvpn.conf" do
      ...
      context ':statusfile param is an array' do
        ...
        it "creates #{options[:plugin_conf_dir]}/10-openvpn.conf with two :statusfile params" do
          is_expected.to contain_file('openvpn.load').with(
            ensure: 'present',
            path: "#{options[:plugin_conf_dir]}/10-openvpn.conf",
      ...
      context ':ensure => absent' do
        ...
        it "does not create #{options[:plugin_conf_dir]}/10-openvpn.conf" do
          is_expected.to contain_file('openvpn.load').with(
            ensure: 'absent',
            path: "#{options[:plugin_conf_dir]}/10-openvpn.conf",
          )
