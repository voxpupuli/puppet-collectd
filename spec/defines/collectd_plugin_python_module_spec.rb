...
describe 'collectd::plugin::python::module', :type => :define do
  ...
  context 'spam module' do
    ...
    it 'imports spam module' do
      should contain_concat__fragment('collectd_plugin_python_conf_spam').with(:content => /Import "spam"/,
                                                                               :target  => '/etc/collectd/conf.d/python-config.conf',)
    end

    it 'includes spam module configuration' do
      should contain_concat__fragment('collectd_plugin_python_conf_spam').with(:content => /<Module "spam">/,
                                                                               :target  => '/etc/collectd/conf.d/python-config.conf',)

      should contain_concat__fragment('collectd_plugin_python_conf_spam').with(:content => /spam "wonderful" "lovely"/,)
    end
